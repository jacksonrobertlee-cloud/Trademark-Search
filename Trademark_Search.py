...
def search_uspto_api(markname: str, api_key: str = "", rows: int = 20):
    ...
    Subscribe to Basic (free) plan and copy your X-RapidAPI-Key.

    Endpoint docs:
      GET /v1/trademarkSearch?keyword=<mark>&searchType=exactMatch
    """
    if not api_key:
        raise ValueError(
    ...
    # Search 1: exact/phrase match
    for search_type in ["exactMatch", "similarMark"]:
        try:
            params = {
                "keyword":    markname,
                "searchType": search_type,
                "rows":       min(rows, 10),  # RapidAPI max per call
                "start":      0,
            }
            ...
            raise ValueError(f"RapidAPI search error: {e}")

    # De-duplicate by serialNumber
    seen = set()
    unique = []
    for item in all_items:
        key = item.get("serialNumber", item.get("markLiteralText", str(item)))
        if key not in seen:
            seen.add(key)
            unique.append(item)
