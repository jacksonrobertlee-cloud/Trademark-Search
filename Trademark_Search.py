...
def search_uspto_api(markname: str, api_key: str = "", rows: int = 100):
    ...
    The ODP key from data.uspto.gov is for PATENT data only and will 403 here.
    """
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            raise ValueError(
                "403 Forbidden from USPTO Trademark API. "
                "Note: The ODP key from data.uspto.gov is for PATENT data only — "
                "it cannot be used here. The Trademark Search API is public and needs no key. "
                "If this persists, check https://developer.uspto.gov/status"
            )
        elif response.status_code == 429:
            raise ValueError(
                "429 Too Many Requests — USPTO public rate limit exceeded. "
...
def main():
    ...
    with st.sidebar:
        ...
        st.success("✅ USPTO Trademark API — No key required")
        st.caption(
            "The USPTO Trademark Search API (api.uspto.gov) is a free public endpoint. "
            "Note: The ODP key from data.uspto.gov is for patents only and is not used here."
        )
        api_key = ""  # Not needed for trademark search
