...
# ── API ────────────────────────────────────────────────────────────────────────
API_BASE_URL        = "https://uspto-trademark.p.rapidapi.com/v1/trademarkSearch"
...
def load_api_key():
    ...
    for secret_name in ("RAPIDAPI_KEY", "USPTO_API_KEY"):
        if secret_name in st.secrets:
            return st.secrets[secret_name].strip()
    # Check environment variables
    for env_name in ("RAPIDAPI_KEY", "USPTO_API_KEY"):
...
# ── USPTO search via RapidAPI ──────────────────────────────────────────────────
def search_uspto_api(markname: str, api_key: str = "", rows: int = 20):
    """
    Search USPTO trademark data via the RapidAPI USPTO Trademark endpoint.

    Uses two complementary endpoints:
      POST /v1/trademarkSearch  — keyword search (form-encoded)
      POST /v1/suggestClass     — class suggestion for goods/services

    Free tier: 500 requests/month
    Sign up at: https://rapidapi.com/pentium10/api/uspto-trademark
    """
    if not api_key:
        raise ValueError(
            "RapidAPI key required.\n\n"
            "Get a free key (500 searches/month):\n"
            "1. Go to https://rapidapi.com/pentium10/api/uspto-trademark\n"
            "2. Sign up and subscribe to the Basic (free) plan\n"
            "3. Copy your X-RapidAPI-Key and paste it in the sidebar"
        )
    ...
    # RapidAPI uses POST with form-encoded body
    search_endpoint = "https://uspto-trademark.p.rapidapi.com/v1/trademarkSearch"

    for search_type in ["exactMatch", "similarMark"]:
        try:
            data = {
                "keyword":    markname,
                "searchType": search_type,
            }
            resp = requests.post(search_endpoint, headers=headers, data=data, timeout=30)

            if resp.status_code == 200:
                result = resp.json()
                # Response can be list, dict with "items", or dict with "hits"
                if isinstance(result, list):
                    items = result
                elif isinstance(result, dict):
                    items = (
                        result.get("items")
                        or result.get("hits", {}).get("hits", [])
                ...
                raise ValueError(
                    "403 Forbidden — Invalid or expired RapidAPI key.\n"
                    "Check your key at https://rapidapi.com/developer/dashboard"
                )
            elif resp.status_code == 429:
                raise ValueError(
                    "429 Too Many Requests — Free tier limit reached (500/month).\n"
                    "Upgrade at https://rapidapi.com/pentium10/api/uspto-trademark"
                )
            elif resp.status_code == 401:
                raise ValueError(
                    "401 Unauthorized — Missing or invalid RapidAPI key.\n"
                    "Paste your X-RapidAPI-Key in the sidebar."
                )
            else:
            ...
            raise ValueError("Request timed out — RapidAPI may be slow. Try again.")
        except requests.exceptions.ConnectionError:
            raise ValueError("Cannot reach RapidAPI. Check your internet connection.")
        except Exception as e:
            raise ValueError(f"RapidAPI search error ({search_type}): {e}")

        time.sleep(0.3)  # Respect rate limit between calls

    # De-duplicate by serialNumber
    seen = set()
    unique = []
    for item in all_items:
        uid = item.get("serialNumber", item.get("markLiteralText", str(item)))
        if uid not in seen:
            seen.add(uid)
            unique.append(item)
...
def suggest_classes_rapidapi(goods_desc: str, api_key: str) -> List[str]:
    """
    Use RapidAPI /v1/suggestClass to recommend international classes
    based on a goods/services description. Bonus feature shown in sidebar.
    """
    if not api_key or not goods_desc:
        return []
    try:
        resp = requests.post(
            "https://uspto-trademark.p.rapidapi.com/v1/suggestClass",
            headers={
                "x-rapidapi-key":  api_key,
                "x-rapidapi-host": RAPIDAPI_HOST,
                "Content-Type":    "application/x-www-form-urlencoded",
            },
            data={"description": goods_desc},
            timeout=20,
        )
        if resp.status_code == 200:
            result = resp.json()
            # Returns list of class codes or objects
            classes = []
            items = result if isinstance(result, list) else result.get("classes", [])
            for item in items:
                if isinstance(item, dict):
                    code = item.get("classCode", item.get("code", ""))
                else:
                    code = str(item)
                if code:
                    classes.append(str(code).zfill(3))
            return classes[:5]
    except Exception:
        pass
    return []
...
def normalize_payload(payload: Dict):
    """
    Normalize RapidAPI USPTO trademark response.
    RapidAPI returns a list directly or under 'items'.
    Field names differ slightly from the old api.uspto.gov endpoint.
    """
    if isinstance(payload, list):
        results = payload
    else:
        results = payload.get("items", payload.get("results", []))

    normalized = []
    for item in results:
        # RapidAPI field names
        classes_raw = (
            item.get("classifications", None)
            or item.get("internationalClasses", [])
            or item.get("classDescriptions", [])
        )
        if isinstance(classes_raw, list):
            class_codes = []
            for c in classes_raw:
                if isinstance(c, dict):
                    code = c.get("classCode", c.get("code", ""))
                else:
                    code = str(c)
                if code:
                    class_codes.append(str(code).zfill(3))
            class_text = ", ".join(class_codes) if class_codes else ""
        else:
            class_text = str(classes_raw)

        goods = (
            item.get("goodsServices", "")
            or item.get("goods", "")
        ...
        normalized.append({
            "mark":   item.get("markLiteralText", item.get("mark", item.get("keyword", ""))),
            "goods":  goods,
            "class":  class_text,
            "status": status,
            "owner":  owner,
...
def main():
    ...
    with st.sidebar:
        ...
        if not api_key:
            api_key = st.text_input(
                "RapidAPI Key",
                type="password",
                help="Your X-RapidAPI-Key for the USPTO Trademark API",
            )
            st.caption(
                "Get a **free key** (500 searches/month): "
                "[rapidapi.com/pentium10/api/uspto-trademark]"
                "(https://rapidapi.com/pentium10/api/uspto-trademark) → "
                "Subscribe to Basic plan → copy X-RapidAPI-Key."
            )
        else:
    ...
    with col2:
        ...
        if st.button("🤖 Auto-suggest Classes", help="Uses AI to suggest classes from your goods description"):
            if goods and api_key:
                with st.spinner("Suggesting classes..."):
                    suggested = suggest_classes_rapidapi(goods, api_key)
                if suggested:
                    st.success(f"Suggested: {', '.join(suggested)}")
                    st.caption("Add these to the Classes selector above if relevant.")
                else:
                    st.info("No suggestions returned. Try a more detailed goods description.")
            elif not goods:
                st.warning("Enter a Goods/Services Description first.")
            else:
                st.warning("Enter your RapidAPI key in the sidebar first.")
    ...
    if not mark.strip():
        ...
        return
    if not api_key:
        st.error("Please enter your RapidAPI key in the sidebar to search trademark records.")
        st.info(
            "Get a free key at https://rapidapi.com/pentium10/api/uspto-trademark — "
            "subscribe to the Basic plan (500 searches/month free)."
        )
        return
    if not classes:
